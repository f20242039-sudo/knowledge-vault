...
with st.sidebar:
    ...
    if is_groq:
        ...
        st.markdown(
            "<div style='padding:0 0.2rem;color:#7c5cbf;font-size:0.75rem;margin-top:0.4rem;line-height:1.7'>"
            "Make sure Ollama is running locally and the model is pulled.<br>"
            "<code style='font-size:0.7rem;color:#9333ea'>ollama pull llama3.2</code>"
            "</div>",
            unsafe_allow_html=True,
        )
...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    if uploaded_file:
        ...
        if st.button("✦  Analyse & Add to Vault", type="primary"):
            ...
            try:
                with st.spinner(f"Reading and analysing with {provider_label}… (15–20 seconds)"):
