...
with st.sidebar:
    ...
    # ── AI Provider ────────────────────────────────────────────────────────────
    st.markdown("---")
    st.markdown("<div style='font-size:0.65rem;font-weight:600;letter-spacing:0.14em;color:#6b5a8e;text-transform:uppercase;margin-bottom:0.5rem'>AI Provider</div>", unsafe_allow_html=True)
    ...
    if is_groq:
        # BYOK — prefer sidebar input, fallback to .env
        import os
        ...
        groq_key = st.text_input(
            ...
            type="password",
            placeholder="gsk_…  (or set GROQ_API_KEY in .env)",
            help="Your key is only stored in session memory and never sent anywhere except Groq.",
        )
        ...
        groq_model = st.text_input(
            ...
            help="Any model available on your Groq account.",
        )
        st.session_state.groq_model = groq_model

        if groq_key:
            st.markdown("<div style='color:#6ee7b7;font-size:0.75rem;margin-top:0.3rem'>✓ Key set</div>", unsafe_allow_html=True)
        else:
            st.markdown("<div style='color:#f87171;font-size:0.75rem;margin-top:0.3rem'>⚠ No key — add one above or set GROQ_API_KEY in .env</div>", unsafe_allow_html=True)
        ...
        ollama_model = st.text_input(
            ...
            help="Any model you have pulled in Ollama (e.g. mistral, llama3.2, phi3).",
        )
        st.session_state.ollama_model = ollama_model

        st.markdown(
            "<div style='color:#9b8db5;font-size:0.75rem;margin-top:0.3rem;line-height:1.6'>"
            "Make sure Ollama is running locally and the model is pulled.<br>"
            "<code style='font-size:0.7rem;color:#7c3aed'>ollama pull llama3.2</code>"
            "</div>",
            unsafe_allow_html=True,
        )
...
def _get_provider_cfg() -> dict:
    """Build provider config dict from sidebar session state."""
...
if page == "Upload":
    ...
    if uploaded_file:
        ...
        if st.button("✦ Analyse & Add to Vault", type="primary"):
            ...
            try:
                with st.spinner(f"Analysing with {provider_label}… this takes about 15 seconds"):
